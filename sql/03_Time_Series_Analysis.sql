select * from nyc_yellow_taxi_trips limit 20;
show time zone;

-- convert from one time zone to another 
select tpep_pickup_datetime, tpep_pickup_datetime at time zone 'pst' from nyc_yellow_taxi_trips;

-- view the list of timezones and their abbreviations in postgres
...
-- Return only the timestamp portion of the current system time
select current_time,
    localtime,
	timeofday();
...
-- return part of a date using date_part
select date_part('day', current_timestamp);
select date_part('month', current_timestamp);
select date_part('hour', current_timestamp);

-- return part of a date using extract
select extract('month' from current_timestamp);

-- using date_part and extract with intervals
...
-- create timestamp from separate date and time components;
select date '2020-09-01' + time '03:00:00' as timestamp;
select make_timestamptz (2020,09,01,03,00,00);

-- create date by concatenating the parts and casting to_date
select to_date(concat(2020,'-',09,'-',01),'yyyy-mm-dd');
select concat(2020,'-',09,'-',01)::date;

--DATE MATH
-- find days elapsed between two dates
select date('2020-06-30') - date('2020-05-31');

-- find months elapsed between two dates
select (extract (month from age('2021-06-30', '2020-01-01')));

-- find difference in calendar-aware differences using age
select (extract(year from age('2022-06-30','2020-01-01')) * 12) + 
 extract (month from age('2022-06-30','2020-01-01'));

-- add 7 days to a date using intervals
select date('2020-06-01') + interval '7 days' as new_date; 

-- adding time interval to times
select time '05:00' + interval '3 hours' as new_time;

--subtracting intervals from times
select time '05:00' - interval '3 hours' as new_time;

-- also possible to subtract two times
select tpep_dropoff_datetime - tpep_pickup_datetime as travel_duration
from nyc_yellow_taxi_trips;

-- intervals can also be multiplied yielding a time value
